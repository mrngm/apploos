...
<html lang="nl">
  ...
  <body>
    ...
    {{- range $index, $day := $schedule.Days -}}
      ...
      {{- $currentDayId := $day.Id }}
      <section class="{{ if isRoze $day.Date -}} roze {{ end }}day {{- if not $isProd }} syncscroll {{ end }}" id="day-{{ $currentDayNumber }}" {{- if not $isProd }} name="daysection" {{ end }}>
