...
var (
	VierdaagseStartTime = time.Date(2026, 7, 18, ROLLOVER_HOUR_FROM_START_OF_DAY, 0, 0, 0, time.FixedZone("CEST", 2*60*60))
	VierdaagseEndTime   = time.Date(2026, 7, 24, ROLLOVER_HOUR_FROM_START_OF_DAY, 0, 0, 0, time.FixedZone("CEST", 2*60*60))
)
...
func DQIToString(issues DQI) string {
	...
	if issues&DQINoGenres != 0 {
		ret = append(ret, "no-genres")
	}
